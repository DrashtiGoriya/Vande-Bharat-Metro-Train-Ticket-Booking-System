import { CalendarCheck, DollarSign, IndianRupee, Ticket } from "lucide-react";
...
const AdminDashboard = () => {
    ...
    xAxis: { categories: bookingRevenue.map((item) => item.name), },
    yAxis: {min: 0,title: { text: "Values" },},

    series: [
        { name: "Booking",data: bookingCount,color: "#6e2e01", },
        {name: "Capacity",data: bookingCapacity,color: "#03431a", },
        {name: "Cancelled",data: cancelCount,color: "#670000", },
        {name: "Revenue",data: bookingRevenue,color: "#011536", }],};
const pieChartOptions = {
    ...
    plotOptions: {
        pie: {innerSize: "60%",dataLabels: {enabled: true,style: { fontWeight: "500" }},},},

    colors: [
        {radialGradient: { cx: 0.5, cy: 0.5, r: 0.7 },stops: [[0, "#6a95eb"],[1, "#0d2a63"],],},
        {radialGradient: { cx: 0.5, cy: 0.5, r: 0.7 },stops: [[0, "#2fdc6b"],[1, "#034f1f"],],},
        {radialGradient: { cx: 0.5, cy: 0.5, r: 0.7 },stops: [[0, "#edad64"],[1, "#5a2704"],],},],
    ...
    const loadEvents = async () => {
        ...
        const modifiedStats = dashBoardStatasctics.map((item) => {
            ...
            return item})
        const bookingCountData = allEvents.map(event => ({name: event.title,y: Number(event.bookingCount)}));

        const cancelledCountData = allEvents.map(event => ({name: event.title,y: Number(event.cancelledCount)}));

        const bookingCapacityData = allEvents.map(event => ({name: event.title,y: Number(event.capacity)}));

        const bookingPriceData = allEvents.map(event => ({name: event.title, y: Number(event.bookingPrice)}));
        ...
        const chartData = [
            { name: "UPCOMING", y: statusCount.UPCOMING },{ name: "LIVE", y: statusCount.LIVE },{ name: "COMPLETED", y: statusCount.COMPLETED },];
                            ...
                            <div className="w-full">
                                <div className="flex w-full justify-between items-start">
                                    <div className={`w-10 h-10 ${item.iconBg} rounded-xl flex items-center justify-center mb-2`}>
                                        <Icon className="text-white" size={20} /></div></div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                <div className="bg-white rounded-2xl shadow-sm p-5">
                    ...
                    {bookingCapacity.length == 0 && bookingCount.length == 0 && bookingRevenue.length == 0 && cancelCount.length == 0
                        ...
                        :
                        <HighchartsReact highcharts={Highcharts} options={barChartOptions} />}</div>

                <div className="bg-white rounded-2xl shadow-sm p-5">
                    <h3 className="font-semibold mb-1">Event Status</h3>
                    <p className="text-sm text-gray-500 mb-4">Distribution overview</p>
                    <HighchartsReact highcharts={Highcharts} options={pieChartOptions} /></div></div>
            ...
            {bookings.length > 0 && <div>
                <div className="flex justify-between items-center mb-3">
                    ...
                    <button className="text-orange-500 text-sm cursor-pointer font-medium"
                        onClick={() => navigate('/admin/bookings')}>View All →</button> </div>
                        ...
                        <div key={booking.id} className="bg-white rounded-2xl shadow-sm p-4 flex justify-between items-center">
                            <div>
                                ...
                                <p className="text-sm text-gray-500">
                                    {booking.user} • {booking.date}</p></div>
